...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(
      ".section-header, .program-card, .service-item, .testimonial-card"
    )
    .forEach((el) => {
      observer.observe(el);
    });
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", function (e) {
      ...
      emailjs
        .sendForm("service_3u4wn5j", "template_ewe4vrg", this)
        .then(
          function () {
            // On Success
            showNotification(
              "Thank you for connecting with us! We look forward to exploring how we can empower your students together.",
              "success"
            );
  ...
  document
    .querySelectorAll(".program-card, .testimonial-card, .service-item")
    .forEach((card) => {
...
document.addEventListener("DOMContentLoaded", () => {
  const scrollContainer = document.querySelector(".programs-scroll");
